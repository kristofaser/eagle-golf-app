...
export default function BookProScreen() {
  ...
  // Sur web, pas de Stripe - on utilisera une solution alternative pour le paiement
  // Pour l'instant, on simule le paiement pour le développement web
  ...
  const { initPaymentSheet, presentPaymentSheet, retrievePaymentIntent } = mockStripeHandlers;

  // Pas de SafeAreaInsets sur web - on utilise un padding fixe
  const webInsets = { top: 0, bottom: 0, left: 0, right: 0 };
  ...
  // Sur web, on affiche un message pour le paiement
  const handlePaymentSuccess = async (paymentIntentId: string) => {
    UniversalAlert.info(
      'Paiement non disponible sur web',
      'Le paiement n\'est pas encore disponible sur la version web. Utilisez l\'application mobile pour effectuer une réservation.'
    );
    router.back();
  };
  ...
  // Sur web, on affiche un message au lieu de traiter le paiement
  const handlePayment = async () => {
    UniversalAlert.info(
      'Paiement non disponible',
      'Le système de paiement n\'est pas encore disponible sur la version web. Veuillez utiliser l\'application mobile Eagle sur iOS ou Android pour effectuer votre réservation.'
    );
    router.back();
  };
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        {/* Header personnalisé comme les autres écrans - pas de padding top sur web */}
        <View style={[styles.header, { paddingTop: webInsets.top }]}>
        ...
        {bookingState.currentStep < 5 && (
          <View style={styles.navigationButtons}>
            ...
            {bookingState.currentStep < 4 ? (
              ...
              // Bouton de paiement - sur web on affiche un message
              <TouchableOpacity
