...
interface EagleLogoProps {
  size?: number;
  variant?: 'primary' | 'white' | 'monochrome' | 'navy';
  animated?: boolean;
  animationType?: 'entrance' | 'pulse' | 'none';
  style?: ViewStyle;
}
...
export const EagleLogo: React.FC<EagleLogoProps> = ({
  ...
  const scale = useSharedValue(animated && animationType === 'entrance' ? 0.8 : 1);
  const opacity = useSharedValue(animated && animationType === 'entrance' ? 0 : 1);

  React.useEffect(() => {
    if (animated && animationType === 'entrance') {
      scale.value = withSpring(1, { damping: 12 });
      opacity.value = withTiming(1, { duration: 400, easing: Easing.out(Easing.ease) });
    } else if (animated && animationType === 'pulse') {
  ...
  const aspectRatio = 841.89 / 786.12;
  const width = size;
