 ...
 */
export function filterGolfsInBounds(golfs: GolfParcours[], region: Region): GolfParcours[] {
  const bounds = getVisibleBounds(region);

  return golfs.filter(
    (golf) =>
      golf.latitude && golf.longitude && isInVisibleBounds(golf.latitude, golf.longitude, bounds)
  );
}
